...
namespace VRC6Bot {
	...
	int GenerateBRR( const s16 *Data, u8 *output, int srclength, int srcloop, int srclooplen, int ffixed, int loopfixed, double amp );
	...
	Sample::Sample( const ITLoader::Sample &source, const ConversionInput::SampleData *cinput ) {
		...
		if( looplength ) {
			if( looplength & 0xF ) {
				...
				if( (looplength * (1+unroll)) < MaxUnrollThreshold ) {
					UnrollLoop( &Data, length, loopstart, looplength, unroll );
				} else {
	...
	int GenerateBRR( const s16 *Data, u8 *output, int srclength, 
			...
			for( ; a < srclength; a += 16 )
			{
				...
				for( b = 0; b < 16; b++ )
				{
					if( (a + b) >= srclength && (a + b) >= 0 )
						...
						sbuffer[ b+2 ] = ClampWord((int)roundf( (double)Data[ a+b ] * amp ));
				}
